...
@section('content')
        ...
        @php    
            ...
            //echo "<pre>";print_r($licensecount);exit;
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('client.update', $client->id) }}"  enctype="multipart/form-data">
                ...
                <div class="col-lg-6 mb-4">
                    ...
                    <input type="text" class="form-control" name="name" id="name" value="{{isset($client->name) ? $client->name: '' }}" required readonly>
                    {{-- {{$client->name}} --}}
                    @error('client')
                        <span>{{ $message }}</span>
                    @enderror
                ...
                <div class="col-lg-6 mb-4">
                    ...
                    <input type="text" name="description" class="form-control"  id="description" cols="" rows="" value="{{isset($client->description) ? $client->description : '' }}" required>
                    {{-- {{$client->description}} --}}
                    @error('description')
                        <span>{{ $message }}</span>
                    @enderror
                ...
                <div class="col-lg-6 mb-4">
                    ...
                    <option value="1" {{ old('status', isset($client) && $client->status == 1 ? 'selected' : '' )}}>Active</option>
                    <option value="2" {{ old('status', isset($client) && $client->status == 2 ? 'selected' : '' )}}>Inactive</option>
                    </select>
                    @error('status')
                        <span>{{ $message }}</span>
                    @enderror
                ...
                <div class="col-lg-4 mb-4">
                    ...
                    <input type="number" class="form-control" name="admin" id="admin" value="{{ $licenseDetails->where('name', 'admin')->first()->lic_count ?? '' }}" min="0" required>
                    @error('admin')
                        <span>{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-4 mb-4">
                    ...
                    <input type="number" class="form-control" name="manager" id="manager" value="{{ $licenseDetails->where('name', 'manager')->first()->lic_count ?? '' }}" min="0" required>
                    @error('manager')
                        <span>{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-4 mb-4">
                    ...
                    <input type="number" class="form-control" name="supervisor" id="supervisor" value="{{ $licenseDetails->where('name', 'supervisor')->first()->lic_count ?? '' }}" min="0" required>
                    @error('supervisor')
                        <span>{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-4 mb-4">
                    ...
                    <input type="number" class="form-control" name="mis" id="mis" value="{{ $licenseDetails->where('name', 'mis')->first()->lic_count ?? '' }}" min="0" required>
                    @error('mis')
                        <span>{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-lg-4 mb-4">
                    ...
                    <input type="number" class="form-control" name="agent" id="agent" value="{{ $licenseDetails->where('name', 'user')->first()->lic_count ?? '' }}" min="0" required>
                    @error('agent')
                        <span>{{ $message }}</span>
                    @enderror
                </div>
