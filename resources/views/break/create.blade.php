...
@section('content')
        ...
        //print_r($breaks[0]);
            // $isEdit = isset($breaks);
            // $route = $isEdit ? route('break.update', isset($breaks->campaign_id) ? $breaks->campaign_id : '') : route('break.store');
            // // route('client.store')
        ...
        @endphp 
<?php 
    // echo "<pre>"; print_r($breakcampaign); exit;
?>
<main id="main">
    <div class="row">
    ...
    <div class="card pt-4">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('break.store') }}"  enctype="multipart/form-data">
                @csrf
                {{-- @if($isEdit)
                        @method('PUT')
                @endif --}}
            ...
            <div class="form-group campaignname">
                <select name="campaign" id="campaign" class="form-control" required>
                    ...
                    @foreach ($campaignlist as $campaignId => $campaignName)
                        ...
                        {{-- {{ $breaks->campaign_id === $campaignId ? 'selected' : '' }} --}}
                    @endforeach
                </select>
                </div>
            </div>
...
//} else { ?>
    ...
    <div class="text-center">
        <button class="btn btn-primary submitbutton mt-4" type="submit" disabled>Create</button>
    </div> 
...
<script>
    ...
    $('#campaign').change(function() {
            ...
            var breakcampaignlist = @json($breakcampaign);
            console.log(breakcampaignlist);
            var breakFound = breakcampaignlist.includes(parseInt(campaign));
            if (breakFound === false) {
                $('.submitbutton').prop('disabled', false);
            } else {
                swal.fire('Break is already exist.');
