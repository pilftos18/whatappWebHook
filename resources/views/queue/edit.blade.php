...
<main id="main">
    ...
    <div class="card pt-4">
        <div class="card-body">
            ...
            @endif
            <!-- create.blade.php -->
            <form method="POST" action="{{  route('queue.update', $queue->id) }}"  enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    {{-- @if($isEdit)
                            @method('PUT')
                        ...
                        <div class="row justify-content-center align-items-center form-box">
                            <div class="col-lg-4">
                                <div class="form-group field-vaanicampaignqueue-queue">
                                    ...
                                    <input type="text" name="queuename" value="{{$queue->queuename}}" id="queuename" class="form-control" required>
                                    @error('queuename')
                                @enderror
                                </div>
                            </div>
                            ...
                            <div class="col-lg-3">
                                <div class="form-group field-vaanicampaignqueue-user">
                                    ...
                                    <select name="VaaniCampaignqueue[user][]" class="form-control select2-multiple" id="vaanicampaignqueue-user" multiple required>
                                        ...
                                        <option value="{{ $user->id }}" @if(in_array($user->id, $mapped_user)) selected @endif>{{ $user->name }}</option>
                                    @endforeach
                            ...
                            <div class="col-lg-3">
                                ...
                                <select id="status" class="form-control" name="status" required>
                                    <option value="1" {{ old('status', isset($queue) && $queue->status == 1 ? 'selected' : '' )}}>Active</option>
                                    <option value="2" {{ old('status', isset($queue) && $queue->status == 2 ? 'selected' : '' )}}>Inactive</option>
                                    </select>
                                </select>
                                @error('status')
                                    <span>{{ $message }}</span>
