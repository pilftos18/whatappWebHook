...
{   
    ...
    public function signin(Request $request)
    {
        ...
        if($userExist)
        {
            ...
            if($userToCheck){
                // $userRole='role';
                $userRole = $userToCheck->role;
                // Build the logout URL with parameters
                $logoutUrl = asset("signoutuser/{$userID}/{$userRole}/{$clientid}");
                $flashMessage = "You are already logged in. <a href='{$logoutUrl}'>Click here to logout</a>.";

                // Set the flash message with the logout URL
                return redirect('/login')->with('already_login', html_entity_decode($flashMessage));
                // if ($userExist->session_identifier) {
                // }
            ...
            if($userRole=='user'){
                ...
                if ($getAllCampaign->count() < 1) {
                    $flashMessage = "No campaign has been assigned to you. Please contact your administrator.";

                    // Set the flash message with the logout URL
                    return redirect('/login')->with('no_campaign', html_entity_decode($flashMessage));
                }
            }
    ...
    public function signoutuser($userid,$role,$clientid)
    {
        /*  $dummyRequest = new Request([
                    'agentid' => $userid,
                    'agentrole' => $role, // or any appropriate role
                    'clientid' => $clientid
                    // Add other required request parameters here
                ]);
                $this->signout($dummyRequest);
    ...
    public function signoutuser_admin($userid,$role,$clientid)
    {
		$sessionData = Session::get('data');
    ...
    public function isSessionActive()
    {
        ...
        $sessionData = session('data', []);
        $userSessionID = $sessionData['userSessionID'] ?? null;
