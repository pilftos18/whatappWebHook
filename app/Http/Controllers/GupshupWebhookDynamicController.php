...
class GupshupWebhookPanasonicController extends Controller
{
    ...
    {		
		...
		$log->debug(__line__." ---- Welcome to handleWebhook Function ------- ");
		$data = $request->all();
		...
		$log->debug(__line__.'Row Data : ' . print_r($data, true));
		...
		try {
			...
			$campaign_data = DB::table('campaign')
			 ...
			 $log->debug(__LINE__.' Campaign Data : ' . print_r($campaign_data, true));
			...
			if ($eventType === 'message') {	
				if (preg_match('/image|document|voice|audio|video/', $type)) 
				{
					...
					$log->debug(__LINE__.'Interactive Data: '.print_r($interactiveData, true));
				}
			}
		...
		if((($chatDetails['assigned_to'] == null || empty($chatDetails['assigned_to'])) && strtolower($text) == 'end' && $chatDetails['is_closed'] == '1') || $opID == 'end')
			{					
				...
				$log->debug(__LINE__.'End the chat ID : '.$chatID);				
			} 
			...
			{				
				...
				{	
					...
					if($templates)
					{
						$log->debug(__LINE__." Inside Template");	
						...
						$this->sendWPMsg($reqData, true,'wp_webhooks', $crudDetails);
					}
					else{						
						$log->debug(__LINE__."No Template Found");	
					}					
					$this->updateSetting($client, $campaign, $chatID, ['lsto' => '']);
				}				
				else if(!in_array($opID, ['mm', 'end', 'ttx']))
				{
					$settingArr = $this->getSetting($client, $campaign, $chatID); // Remove This Letter			
					if($interactiveFlag === true)
					{					
						if(!empty($interactiveData))
						{
							$log->debug(__LINE__.' Inside interactiveData');
							...
							$log->debug(__LINE__.'Interactive Data is empty');
						}
					}
