    ...
    {   
        ...
        if ($request->ajax()) {
           // $data = Users::whereIn('status', [0,1,2])->latest()->get();
            ...
            ->leftJoin('queue','queue_mapping.queue_id','=','queue.id')
            ->select('users.*','clients.name as client',DB::raw('GROUP_CONCAT(queue.queuename) AS queue_names'))
    ...
    {   
        ...
        if ($request->ajax()) {
           // $data = Users::whereIn('status', [0,1,2])->latest()->get();
            ...
            ->whereIn('clients.id',[$client_id])
            ->where('clients.deleted_at', 1)
            ->whereNotIn('users.role', ['super_admin', 'manager'])
            ->groupBy('users.id','clients.name')
            ->latest()
            ->get();
