...
class ChatController extends Controller
{
    ...
    //added by pooja 03-09-2024
    public function showAll($mobile)
    {

        $get_ids = Chats::select('chats.id')
        ...
        $chat = Chat_log::whereIn('chat_id',$all_chat_ids['id'])->select('chat_id','in_out','message','media_path','timestamp')->get();
    //    $chat = Chat_log::where('chat_id',$id)
    //     ->select('chat_id','in_out','message','media_path','timestamp')
    //     ->get();

        return view('show_chat', ['chat' => $chat,'mobile'=>$mobile]);
    }
    ...
    public function getChatDetails(Request $request){
        ...
        //changes by pooja 03-09-2024
        $get_ids = Chats::select('chats.id')
            ->join('clients', 'chats.client_id', '=', 'clients.id')
            ->join('campaign', 'chats.campaign_id', '=', 'campaign.id')
            ->where('chats.status', 1)
            ->where('chats.cust_unique_id', $request['mobile'])
            ->pluck('chats.id');
    ...
    //    print_r($sessionData);exit;
        ...
        {			
            ...
            // ->get();
            // $chats = Chats::select('chats.id,chats.client_id,chats.unique_id,chats.campaign_id,chats.org_no,chats.cust_unique_id,chats.customer_name,chats.chat_log_path,chats.assigning_flag,chats.req_assigning_at,chats.open_at,chats.assigned_to,chats.assigned_at,chats.assigned_by,chats.is_closed,chats.closed_at,chats.closed_by,chats.dispo,chats.sub_dispo,chats.remark,chats.status,chats.is_callback,chats.callback_at,chats.lsto,chats.created_at,chats.created_by,chats.updated_at,chats.updated_by,chats.vip_session,campaign.interaction_per_user')
            // ->leftJoin('campaign', 'chats.campaign_id', '=', 'campaign.id')
            // ->where('chats.assigned_to', $sessionData['userID'])
            // ->where('chats.client_id', $sessionData['Client_id'])
            // ->where('chats.campaign_id', $sessionData['campaignID'])
            ...
            /* if($sessionData['userID'] == '75')
			{
				echo "<pre>"; print_r($chatlogArray);
				echo "<pre>"; print_r($chatlogArray2);die();
			} */
...
// if($sessionData['userID'] == '75')
// {
	// echo "<pre>newchats"; print_r($newchats);
	// echo "<pre>get_ids"; print_r($get_ids);
	// echo "<pre> sessionData"; print_r($sessionData);die();
// }
            $newchatLogs = Chat_log::whereIn('chat_id',$all_chat_ids['id'])->orderBy('id', 'asc')->get();
    ...
    {   
        ...
        if (session()->has('data')) {			
            ...
            //pooja 1-10-2024
            // $newchatLogs = Chat_log::whereIn('chat_id',$all_chat_ids['id'])->get();
            // $newchatLogsCount = Chat_log::whereIn('chat_id',$all_chat_ids['id'])->count();
    ...
    {   
        ...
        $sessionData = Session::get('data');
        $agent_log->debug(__LINE__."session data".json_encode($sessionData));
...
//  $NewchatIds = $chatLogs->pluck('chat_id')->toArray();
        ...
        // start code by mahesh 14 jan 2024
            ...
            // update end time for every 5 seconds login and break where activity_status is 1 code by mahesh start
