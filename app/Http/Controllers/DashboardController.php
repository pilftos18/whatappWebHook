...
class DashboardController extends Controller
{
    ...
    public function getadminDashboardList(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (isset($sessionData) && $sessionData['userRole'] == 'admin' || $sessionData['userRole'] == 'supervisor' || $sessionData['userRole'] == 'mis') {
                ...
                ->groupBy('users.id', 'users.name', 'campaign.name', 'users.login_status', 'users.created_at')
                ->get();
    ...
    public function fetch_dashboard_data(Request $request)
    {
        ...
        $agent_log->debug(__LINE__."\n\n\n-------------------- fetch_dashboard_data Funtion Start  ------------------------------------");
        ...
        if(session()->has('data'))
        {
            ...
            // update end time for every 5 seconds login and break where activity_statu is 1 code by mahesh start
                ...
                if($userRole=='user'){
                    ...
                    ->first();
                    $agent_log->debug(__LINE__.'\n lastBreakLog1 data : ' . json_encode($lastBreakLog1));
                    ...
                    $agent_log->debug(__LINE__.'\n breakLogsToUpdate data : ' . json_encode($breakLogsToUpdate));
                    // // Check if there is an existing break log record that needs to be updated
            ...
            if(isset($sessionData['campaignID'])){
                ...
                $agent_log->debug(__LINE__.'\n getAllCampaign data : ' . json_encode($getAllCampaign));
                if($getAllCampaign){
                    if ($getAllCampaign->count() === 1) {
                        ...
                        if ($updateCampaign > 0) {
                            $agent_log->debug(__LINE__.'\n Campaign id and name updated in user table  : ' . $updateCampaign);
                            $agent_log->debug(__LINE__.'\n User data updated', [
                                'user_id' => $userID,
                                'campaign_id' => $campaignID,
                                'campaign_name' => $campaignName,
                    ...
                    }elseif($getAllCampaign->count() > 1){
                        $campMsg = 'Show Campaigns';
                        $agent_log->debug(__LINE__.'\n Show Campaigns ');
                    }else{
                        $campMsg = 'No Campaigns';
                        $agent_log->debug(__LINE__.'\n No Campaigns ');
                    }
                }else{
                    $campMsg = "No Campaigns";
                    $agent_log->debug(__LINE__.'\n No campaign has been assigned to you. Please contact your administrator. ');
                }
            }
            ...
            if($campaignID){
                ...
                $agent_log->debug(__LINE__.'\n Fetch All chats count  data : ' . $allChats);
                ...
                ->count();
                $agent_log->debug(__LINE__.'\n Fetch oldPendingChats chats count  data : ' . $oldPendingChats);
                ...
                ->count();
                $agent_log->debug(__LINE__.'\n Fetch activeChats chats count  data : ' . $activeChats);
                ...
                ->distinct('chats.id')
                ->count();
                $agent_log->debug(__LINE__.'\n Fetch pendingChats chats count  data : ' . $pendingChats);

                $closeChats = Chats::where('is_closed', 2)->where('assigned_to', $userID)->where('client_id', $clientid)->where('campaign_id', $campaignID)->whereDate('closed_at', $today)->count(); // change by gaurav
                ...
                $agent_log->debug(__LINE__.'\n Fetch closeChats chats count  data : ' . $closeChats);
            }
            ...
            $agent_log->debug(__LINE__.'\n fetch old login time for today : ' . json_encode($loginTimeArr));
            ...
            ->latest('created_at')
            ->first();
            $agent_log->debug(__LINE__.'\n fetch current login time for today : ' . json_encode($currentLoginTimeArr));
            ...
            $agent_log->debug(__LINE__.'\n fetch old Ready time for today : ' . json_encode($readyTimeArr));
            ...
            ->latest('created_at')
            ->first();
            $agent_log->debug(__LINE__.'\n fetch current Ready time for today : ' . json_encode($currentReadyTimeArr));
            ...
            $agent_log->debug(__LINE__.'\n fetch old Pause time for today : ' . json_encode($breakTimeArr));
            ...
            ->latest('created_at')
            ->first();
            $agent_log->debug(__LINE__.'\n fetch current Pause time for today : ' . json_encode($currentBreakTimeArr));
            ...
            $agent_log->debug(__LINE__.'\n fetch old Soft break time for today : ' . json_encode($softTimeArr));
            ...
            ->latest('created_at')
            ->first();
            $agent_log->debug(__LINE__.'\n fetch current Soft break time for today : ' . json_encode($currentSoftTimeArr));
            ...
            $logMessage = __LINE__ . " : getting dashboard data   --";
            $logMessage .= "\nAll Chats: " . $allChats;
            $logMessage .= "\nActive Chats: " . $activeChats;
            $logMessage .= "\nOld Pending Chats: " . $oldPendingChats;
            $logMessage .= "\nPending Chats: " . $pendingChats;
            $logMessage .= "\nClose Chats: " . $closeChats;
            $logMessage .= "\nTotal login Time : " . $totalTimeString;
            $logMessage .= "\nTotal login Time : " . $CRDtotalTimeString;
            $logMessage .= "\nTotal login Time : " . $CBRtotalTimeString;
            $logMessage .= "\nTotal login Time : " . $IDLtotalTimeString;
            $logMessage .= "\nSoft Time: " . $CSFtotalTimeString;
            $logMessage .= "\nCamp Msg: " . $campMsg;
            $logMessage .= "\nDashboard Array: " . json_encode($dashboardArray); 
            $agent_log->debug($logMessage);
            $agent_log->debug(__LINE__."\n\n\n-------------------- fetch_dashboard_data Funtion END  ------------------------------------");
    ...
    public function refresh_break_time(Request $request)
    {
        DB::enableQueryLog();
        $agent_log = Log::channel('agent_log');
        $agent_log->debug(__LINE__."\n\n\n-------------------- refresh_break_time Funtion Start  ------------------------------------");
        ...
        if(session()->has('data'))
        {
            ...
            $logMessage = __LINE__ . " : Refresh break time send data   --";
            $logMessage .= "\nBreak Existing Time: " . $existingTime;
            $logMessage .= "\nBreak Name: " . $break_name;
            $logMessage .= "\nLog Message: " . json_encode($logMessage); 
            $agent_log->debug($logMessage);
            $agent_log->debug(__LINE__."\n\n\n-------------------- refresh_break_time Funtion END  ------------------------------------");
            return response()->json($dashboardArray);
        }else{
            return view('login');
        } 
    ...
    public function check_admin_is_login(Request $request)
    {
    ...
    public function setCampaignSession(Request $request)
    {
        DB::enableQueryLog();
        $agent_log = Log::channel('agent_log');
        $agent_log->debug(__LINE__."\n\n\n-------------------- setCampaignSession Funtion Start  ------------------------------------");
        ...
        if (isset($campaignID) && !empty($campaignID)) {
            ...
            if($campaignID){
                ...
                $agent_log->debug(__LINE__.'\n Fetch Old Pending Chats count  data : ' . $oldPendingChats);
               ...
               // $pendingChats = Chats::where('is_closed', 1)->where('assigned_to', $userID)->where('client_id', $clientid)->where('campaign_id', $campaignID)->where('is_read', 1)->whereDate('created_at', $today)->count();
                ...
                ->distinct('chats.id') // Ensuring distinct records based on 'chats.id'
                ->count();
                $agent_log->debug(__LINE__.'\n Fetch Active chats count  data : ' . $activeChats);
                ...
                ->distinct('chats.id') // Ensuring distinct records based on 'chats.id'
                ->count();
                $agent_log->debug(__LINE__.'\n Fetch Pending chats count  data : ' . $pendingChats);
            ...
            $logMessage = __LINE__ . " : Set Campaign data   --";
            $logMessage .= "\nAll Chats: " . $allChats;
            $logMessage .= "\nActive Chats: " . $activeChats;
            $logMessage .= "\nOld Pending Chats: " . $oldPendingChats;
            $logMessage .= "\nPending Chats: " . $pendingChats;
            ...
            $logMessage .= "\nLog Message: " . json_encode($logMessage); 
            $agent_log->debug($logMessage);
            $agent_log->debug(__LINE__."\n\n\n-------------------- setCampaignSession Funtion END  ------------------------------------");
            return response()->json($dashboardArray);
        }
