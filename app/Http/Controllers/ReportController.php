        ...
        //echo "<pre>";print_r($campaign);die;  
            if($user[0] === "All"){
                ...
                ->select(
                    ...
                    'break_log.break_name',
                    'break_log.start_time',
                    'break_log.end_time',
                    'break_log.break_time',
                    'break_log.activity_status'
                )
                ...
                ->where('break_log.break_name', 'Login')
            ...
            // Loop through the 'Login' records
            foreach ($loginData as $loginRow) {
                // Fetch data for other break types
                $otherBreakData = DB::table('break_log')
                    ->select(
                        ...
                        DB::raw('SEC_TO_TIME(SUM(CASE WHEN break_log.break_name IN ("Tea","Bio","Lunch","Meeting") THEN TIME_TO_SEC(break_log.break_time) ELSE 0 END)) AS Break_Duration'),
                    ...
                    ->where('break_log.client_id', $client_id)
                    ->whereIn('break_log.user_id', $userslist)
                    //->where('break_log.break_name', '!=', 'Login') // Exclude 'Login' break
                    ->where('break_log.start_time', '>=', $loginRow->start_time)
                    ->where('break_log.end_time', '<=', isset($loginRow->end_time) && ($loginRow->end_time) != null  ? $loginRow->end_time : now())
                    ->orderBy('break_log.user_id')
                    ->orderBy('break_log.start_time', 'desc')
                    ->get();
            ...
            // if ($userRole == 'super_admin') {
                ...
                $csvarray[] = ['Client','Agent Name', 'Agent id','Login time', 'Logout time', 'Total Login Duration','Break Duration','Lunch', 'Tea','Bio','Meeting','Soft break','Ready Duration','Ideal Duration'];


            foreach ( $breakData as $k => $row) {
                //var_dump($row);
    ...
    {   
        ...
        $csvarray = [];
            ...
            // if ($userRole == 'super_admin') {
                ...
                $csvarray[] = ['chat_id','Client','Campaign','assigned to','Customer name','Customer number', 'Assign status','Assigned by','Is closed','Closed by','Disposition','Sub_disposition','Remark','Assigned at','Created at','Closed At'];
