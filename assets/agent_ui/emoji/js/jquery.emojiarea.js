...
var EmojiArea = function () {
  function EmojiArea(emojiArea, options) {
    ...
    if (options.type !== 'unicode') {
      this.$ti.hide();
      this.$e = (0, _jquery2.default)('<div>').addClass('emoji-editor').attr('tabIndex', 0).attr('placeholder','Send your message...').attr('contentEditable', true).text(this.$ti.text()).on(options.inputEvent, this.onInput.bind(this)).on(options.keyEvent, this.onKey.bind(this)).on('copy', options.textClipboard ? this.clipboardCopy.bind(this) : function () {
        return true;
